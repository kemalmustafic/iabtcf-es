...
describe("CmpApi", (): void => {
  ...
  it("should override the stub", async (): Promise<void> => {
    ...
    expect(window[API_KEY], `window.${API_KEY} after cmpApi created`).to.be.a(
      "function"
    );
    return new Promise((resolve: () => void): void => {
      window[API_KEY]("ping", 2, (ping: Ping): void => {
        ...
        expect(
          ping.cmpVersion,
          "ping.cmpVersion after cmpApi created"
        ).is.above(0);
        expect(ping.cmpLoaded, "ping.cmpLoaded after cmpApi created").to.be
          .true;
        ...
        expect(
          ping.apiVersion,
          "ping.apiVersion after cmpApi created"
        ).to.equal("2");
        expect(
          ping.displayStatus,
  ...
  it('should set CmpApiModel.displayStatus = "visible" when uiVisible is passed as true to the update function', (): void => {
    ...
    expect(
      ...
      "CmpApiModel.displayStatus - initial"
    ).to.equal(DisplayStatus.HIDDEN);
    ...
    expect(
      CmpApiModel.displayStatus,
      "CmpApiModel.displayStatus - after set cmpApi.uiVisible to true"
    ).to.equal(DisplayStatus.VISIBLE);

    cmpApi.update(TCStringFactory.base(), false);

    expect(
      CmpApiModel.displayStatus,
      "CmpApiModel.displayStatus - after set cmpApi.uiVisible to false"
    ).to.equal(DisplayStatus.HIDDEN);
  });
    ...
    it(`should callback with success=true and result=something if command=${command} and version=${version} because ${because}`, (done: () => void): void => {
      ...
      const callDat = (): void => {
        window[API_KEY](
          command,
          version,
          (result: any, success: boolean): void => {
  ...
  runFailCommand(
    TCFCommand.GET_TC_DATA,
    2.1,
    "version is a floating point number that is greater than 2"
  );
  runFailCommand(
    TCFCommand.GET_TC_DATA,
    "2.1",
    "version is a floating point that doesn't evaluate to 2"
  );
  ...
  runSucceedCommand(
    TCFCommand.GET_TC_DATA,
    2.0,
    "version is a floating point number that evaluates to 2"
  );
  ...
  it("should call a custom command through the page interface with multiple parameters", (done: () => void): void => {
    ...
    window[API_KEY](
      commandName,
      2,
      (param: boolean, param2: string, param3: string): void => {
        expect(param, "param").to.be.a("boolean");
        expect(param, "param").to.equal(passParam);
        expect(param2, "param2").to.be.a("string");
        expect(param2, "param2").to.equal(passParam2);
  ...
  const runBuiltInCustomTest = (command: TCFCommand): void => {
    if (
      ...
      command !== TCFCommand.ADD_EVENT_LISTENER
    ) {
      it(`should call a built-in ${command} command through the page interface and decorate the response in the middleware`, (done: () => void): void => {
        ...
        const cmpApi = getCmpApi(false, {
          [command]: (
            ...
            response: Response | boolean | VendorList,
            success: boolean
          ): void => {
        ...
        window[API_KEY](
          command,
          2,
          (response: Response, success: boolean): void => {
  ...
  Object.keys(TCFCommand).map((key) => runBuiltInCustomTest(TCFCommand[key]));
  ...
  it(`should set gdprApplies=true, displayStatus="${DisplayStatus.DISABLED}", eventStatus="${EventStatus.TC_LOADED}", and cmpStatus="${CmpStatus.LOADED}" on the first set of the tcString when uiVisible=false`, (): void => {
    ...
    expect(CmpApiModel.displayStatus, "CmpApiModel.displayStatus").to.equal(
      DisplayStatus.DISABLED
    );
    expect(CmpApiModel.eventStatus, "CmpApiModel.eventStatus").to.equal(
      EventStatus.TC_LOADED
    );
    expect(CmpApiModel.cmpStatus, "CmpApiModel.cmpStatus").to.equal(
      CmpStatus.LOADED
    );
  });
