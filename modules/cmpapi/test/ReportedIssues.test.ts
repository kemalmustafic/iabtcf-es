...
import { makeRandomInt, TCStringFactory } from "didomi-iabtcf-testing";
...
describe("Reported issues", (): void => {
  ...
  it("Issue 96 CmpApi should respond to addEventListener call with an error object when in an Error state", (done: () => void): void => {
    ...
    window[API_KEY](
      ...
      (response: Response): void => {
        expect(response instanceof Disabled, "response instanceof Disabled").to
          .be.true;
  ...
  it("Should not throw an error if getTCData is called before update is called", (done: () => void): void => {
    ...
    const callDatFunc = (): void => {
      window[API_KEY](TCFCommand.GET_TC_DATA, 2, (response: Response): void => {
        expect(response instanceof TCData, "response instanceof TCData").to.be
          .true;
  ...
  it('126 Should not throw an error tc string is set as ""', (done: () => void): void => {
    ...
    expect(CmpApiModel.tcModel.cmpVersion, "tcModel.cmpVersion").to.equal(
      cmpVersion
    );
  ...
  it("120 AddEventListener: 'cmpuishown' isn't being triggered - test toggles update UIVisible", (done: () => void): void => {
    ...
    window[API_KEY](
      ...
      (tcData: TCData, success: boolean): void => {
        ...
        expect(tcData.eventStatus, `evenStatus #${count}`).to.equal(
          eventStatus
        );
  ...
  it("196 should not call callback twice on addEventListener", (): void => {
    ...
    expect(
      callback.callCount,
      "callback.callCount :: Before CMP API Created"
    ).to.equal(0);
    ...
    expect(
      callback.callCount,
      "callback.callCount :: After CMP API Created"
    ).to.equal(0);
    cmpApi.update(TCStringFactory.base());
    expect(
      callback.callCount,
      "callback.callCount :: After CmpApi.update()"
    ).to.equal(1);
  });
  ...
  it("220 Events are not triggered in addEventListener() after update() in invoked with uiVisible=false", (): void => {
    const spy = sinon.spy();
    const tcString =
      "CO5dPi4O5dPx2B7DvCENA2CgAAAAAAAAAAAAGewAAM8gAAAA.YAAAAAAAAAAA";
    ...
    expect(tcData.eventStatus, `call 0 eventStatus`).to.equal(
      EventStatus.TC_LOADED
    );

    tcData = spy.getCall(1).args[0];

    expect(tcData.eventStatus, `call 1 eventStatus`).to.equal(
      EventStatus.CMP_UI_SHOWN
    );

    tcData = spy.getCall(2).args[0];

    expect(tcData.eventStatus, `call 2 eventStatus`).to.equal(
      EventStatus.USER_ACTION_COMPLETE
    );

    tcData = spy.getCall(3).args[0];

    expect(tcData.eventStatus, `call 3 eventStatus`).to.equal(
      EventStatus.CMP_UI_SHOWN
    );

    tcData = spy.getCall(4).args[0];

    expect(tcData.eventStatus, `call 4 eventStatus`).to.equal(
      EventStatus.USER_ACTION_COMPLETE
    );
  });
});
