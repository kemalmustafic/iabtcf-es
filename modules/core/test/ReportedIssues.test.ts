...
describe("Issues Reported", (): void => {
  it("112 Vendor ranges incorrectly decoded from publisher restrictions", async (): Promise<
    void
  > => {
    ...
    const vendors = newTCModel.publisherRestrictions.getVendors(
      purposeRestriction
    );
  ...
  it("91 TCString.encode use 0 as vendorListVersion instead of gvl", async (): Promise<
    void
  > => {
    const CMPID = makeRandomInt(2, 100);
    const CMPVERSION = makeRandomInt(1, 63);
    ...
    const vendorlist = require("didomi-iabtcf-testing/lib/vendorlist/vendor-list.json");
  ...
  it("77 should not delete tcfPolicyVersion and gvlSpecificationVersion after a language is changed", (done: () => void): void => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const gvl = new GVL(
      require("didomi-iabtcf-testing/lib/vendorlist/vendor-list.json")
    );
    ...
    gvl.changeLanguage(language).then((): void => {
      expect(gvl.tcfPolicyVersion, "gvl.tcfPolicyVersion").to.equal(
        tcfPolicyVersion
      );
      expect(
        gvl.gvlSpecificationVersion,
        "gvl.gvlSpecificationVersion"
      ).to.equal(gvlSpecificationVersion);
      done();
    });
  ...
  it("117 TCString.encode writes all vendors as disclosed even after GVL.narrowVendorsTo([...])", async (): Promise<
    void
  > => {
    ...
    expect(
      tcModel.vendorsDisclosed.size,
      "tcModel.vendorsDisclosed.size"
    ).to.equal(vendors.length);

    const decoded = TCString.decode(TCString.encode(tcModel));

    expect(
      decoded.vendorsDisclosed.size,
      "decoded.vendorsDisclosed.size"
    ).to.equal(vendors.length);
  });
  ...
  it("142 it should throw an error if the bitfield length does not match the maxId", (): void => {
    const str =
      "CLSYjTaOngnCIAOABBENAXCMAGOAABBAAA7IA5n-m7fP6_3fbqVv6E__PoA5Aqff3aJx8tv_1967rfnQEQoAIAAQCrwkAEABAcACABIMACAAuApEVABABUSABgBCAVSAtIoACACIArYQAHACgAFgAVwBJgDcAI7AWgMAAgBiKgAgBMgFfKQAQBRkQAQA4AFiBAAoA-AEVAJlAVgAtYcACAJcAr4eABAK8OgBgFzAOmAqwgABAWyA.IFukWSQh";
  ...
  it("156 it should auto detect 1 version", (): void => {
    const str =
      "BOhwdphOxFC7tAHABBFRC--AAAAuhr_7__7-_9_-_f__9uj3Or_v_f__32ccL59v_h_7v-_7fi_20nV4u_1vft9yfk1-5ctDztp507iakivXmqdeb9v_nz3_5pxP78k89r7337Ew_v8_v-b7BCON_YxEiA";
  ...
  it("162 Legal basis purpose restriction is reflected on vendors without flexible purposes too", async (): Promise<
    void
  > => {
    ...
    expect(
      decodedTCModel.publisherRestrictions.getVendors(purposeRestriction)
    ).be.an("array").that.is.empty;
  });

  it("204 Not possible to set vendorConsent for vendor which declared felxiblePurposes but no purposes", async (): Promise<
    void
  > => {
    ...
    vendorIds.forEach((vendorId: number): void => {
      ...
      if (vendor.purposes.length) {
        expect(
          decodedTCModel.vendorConsents.has(vendorId),
          `decoded tcModel.vendorConsents.has(${vendorId})`
        ).to.be.true;
      } else if (vendor.legIntPurposes.length) {
        expect(
          decodedTCModel.vendorLegitimateInterests.has(vendorId),
          `decoded tcModel.vendorLegitimateInterests.has(${vendorId})`
        ).to.be.true;
      }
    });
  });

  it("191 vendorConsents empty when decoding TCF 1.1 consentString that uses vendorRangeList", async (): Promise<
    void
  > => {
  ...
  it("201 unable to decode valid TCF2 String", (): void => {
    ...
    expect((): void => {
      tcModel = TCString.decode(
        "CO4VGswO4VGswAfZCBDEAzCsAP_AAH_AAAigGUNf_X9fb2vj-_599_t0eY1f9_63t-wzjheMs-8NyZ-X_J4Wv2MyvB34JqQKGRgkunLBAQdtHGncTQgBwIlViTLMY02MjzNKJrJEilsbe2dYGH9vn8XT_ZKZ70-_v__7v3___33_5Ayhr_6_r7e18f3_Pvv9ujzGr_v_W9v2GccLxln3huTPy_5PC1-xmV4O_BNSBQyMEl05YICDto407iaEAOBEqsSZZjGmxkeZpRNZIkUtjb2zrAw_t8_i6f7JTPen39___d-___--__"
      );
    }).not.to.throw();

    expect(tcModel.version, `tcModel.version`).to.equal(2);
    expect(
      tcModel.publisherRestrictions.getVendors(new PurposeRestriction(1, 1)),
      `tcModel.publisherRestrictions vendor array`
