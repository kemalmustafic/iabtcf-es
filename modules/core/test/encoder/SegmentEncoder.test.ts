...
describe("encoder->SegmentEncoder", (): void => {
  ...
  it("TCModel->Core TC String->TCModel and should be equal", async (): Promise<
    void
  > => {
    const tcModel: TCModel = getTCModelWithGVL();
  ...
  it("should encode a OOB Vendors Allowed Segment properly into a string", async (): Promise<
    void
  > => {
    const tcModel: TCModel = getTCModelWithGVL();
    let encoded = "";
    ...
    expect(encoded, "shold not be empty").to.not.equal("");
    const decodedModel = SegmentEncoder.decode(
      encoded,
      new TCModel(),
    ...
    tcModel.vendorsAllowed.forEach((value: boolean, vendorId: number): void => {
      expect(
        decodedModel.vendorsAllowed.has(vendorId),
        `vendorsAllowed.has(${vendorId})`
      ).to.equal(value);
    });
  });

  it("TCModel->OOBAllowed segment->TCModel and should be equal", async (): Promise<
    void
  > => {
  ...
  const testCustomPurposes = (
    msg: string,
    customPurposes?: IntMap<Purpose>
  ): void => {
    it(`TCModel->PublisherTC segment->TCModel and should be equal with ${msg}`, async (): Promise<
      void
    > => {
      const tcModel: TCModel = getTCModelWithGVL();
      const decodedModel: TCModel = new TCModel();
      ...
      if (
        customPurposes !== undefined &&
        Object.keys(customPurposes).length > 0
      ) {
        tcModel.customPurposes = customPurposes;
        const pruposeIds: string[] = Object.keys(customPurposes).sort(
      ...
      expect(tcModel.numCustomPurposes, "tcModel.numCustomPurposes").to.equal(
        numCustomPurposes
      );
      ...
      expect(
        decodedModel.publisherCustomConsents.size,
        "publisherCustomConsents.size"
      ).to.equal(tcModel.publisherCustomConsents.size);
      expect(
        decodedModel.publisherCustomLegitimateInterests.size,
        "publisherCustomLegitimateInterests.size"
      ).to.equal(tcModel.publisherCustomLegitimateInterests.size);
      expect(decodedModel.numCustomPurposes, "numCustomPurposes").to.equal(
        tcModel.numCustomPurposes
      );
    });
  };
