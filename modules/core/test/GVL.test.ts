...
describe("GVL", (): void => {
  ...
  const assertTranslated = (gvl: GVL, lang: string): void => {
    expect(gvl.purposes, "gvl.purposes").to.deep.equal(
      translationJson.purposes
    );
    expect(gvl.specialPurposes, "gvl.specialPurposes").to.deep.equal(
      translationJson.specialPurposes
    );
    expect(gvl.features, "gvl.features").to.deep.equal(
      translationJson.features
    );
    expect(gvl.specialFeatures, "gvl.specialFeatures").to.deep.equal(
      translationJson.specialFeatures
    );
    expect(gvl.stacks, "gvl.stacks").to.deep.equal(translationJson.stacks);

    expect(gvl.purposes, "gvl.purposes").to.not.deep.equal(
      vendorlistJson.purposes
    );
    expect(gvl.specialPurposes, "gvl.specialPurposes").to.not.deep.equal(
      vendorlistJson.specialPurposes
    );
    expect(gvl.features, "gvl.features").to.not.deep.equal(
      vendorlistJson.features
    );
    expect(gvl.specialFeatures, "gvl.specialFeatures").to.not.deep.equal(
      vendorlistJson.specialFeatures
    );
  ...
  it("should get latest GVL if nothing is passed to the constructor", async (): Promise<
    void
  > => {
  ...
  it('should get latest GVL if "LATEST" is passed to the constructor', async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
  ...
  it("should get versioned GVL if version number is passed", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
  ...
  it("should get versioned GVL if version number as string is passed", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
  ...
  it('should not re-request the "LATEST" vendorlist json if it has already downloaded it', async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
  ...
  it("should not re-request a versioned vendorlist json if it has already downloaded it", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
    ...
    expect(req.method).to.equal("GET");
    expect(req.url).to.equal(
      `${GVL.baseUrl}archives/vendor-list-v${gvlVersion}.json`
    );
  ...
  it("should not re-request a vendorlist json if it has been passed in to the constructor", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
  ...
  it("should not re-request a vendorlist json latest is request and then that version is requested later", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
  ...
  it("should remove a vendor if it has a deletedDate", (): void => {
    ...
    expect(json.vendors[vendorId], `json.vendors["${vendorId}"]`).not.to.be
      .undefined;
    const gvl: GVL = new GVL(json);
    expect(gvl.vendors[vendorId], `gvl.vendors["${vendorId}"]`).to.be.undefined;
  });

  it("should replace the language when changeLanguage() is called with valid language", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com";
  ...
  const langNotOk = (language: string): void => {
    it(`should throw an error if ${language} is passed to changeLanguage()`, async (): Promise<
      void
    > => {
      GVL.baseUrl = "http://sweetcmp.com";
  ...
  it("should not re-request the same language file if it has already been loaded", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com";
  ...
  it("should delete a language from the cache when requested", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com";
  ...
  it("should delete a gvl version from the cache when requested", async (): Promise<
    void
  > => {
    GVL.baseUrl = "http://sweetcmp.com/";
