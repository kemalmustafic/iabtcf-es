...
describe("TCModel", (): void => {
  ...
  it("should construct a TCModel with a GVL argument", async (): Promise<
    void
  > => {
    ...
    expect(tcModel.policyVersion, "tcModel.policyVersion").to.equal(
      gvl.tcfPolicyVersion
    );

    expect(tcModel.vendorListVersion, "tcModel.vendorListVersion").to.equal(
  ...
  describe("consentLanguage", (): void => {
    it('should default to "EN" and set and get whatever I want', (): void => {
      ...
      expect(tcModel.consentLanguage, "consentLanguage default").to.equal(
        altLang
      );
    });
  });
